...
function buildShellCommand(cwd, command) {
  ...
  return (
    ...
    `{ ${command}\n}; __xayz_rc=$?; ` +
    `printf '\\n\\001XAYZ_CWD:%s\\001XAYZ_RC:%s\\001' "$PWD" "$__xayz_rc"`
  );
}
  ...
  await new Promise((resolve) => {
    ...
    conn.exec(full, { pty: { cols, rows, term: 'xterm-256color' } }, (err, stream) => {
      ...
      const guard = setTimeout(() => {
        ...
        res.write('\r\n\x1b[33m[dihentikan: batas waktu serverless 55 detik tercapai]\x1b[0m\r\n');
        finish();
      }, Number(process.env.XAYZ_EXEC_TIMEOUT || 55000));
      if (guard.unref) guard.unref();

      stream.on('data', (d) => { try { res.write(d); } catch {} });
      stream.stderr.on('data', (d) => { try { res.write(d); } catch {} });
      stream.on('close', () => { clearTimeout(guard); finish(); });
      stream.on('error', () => { clearTimeout(guard); finish(); });
